...
public class ccStatement extends Visitor{
	 ...
	 */
	public ccStatement(GNode gNode, ccBlock block2, boolean isConditional) {
		block = block2;
		conditionalFlag = isConditional;
		dispatch(gNode);
	}
	...
	public void visitCallExpression(GNode n){
		String methodName = "";
		String variableName = "";
		boolean tempFlag = conditionalFlag;
		if(conditionalFlag){
			line+="({";
		...
		if(n.getString(2).matches("print|println")&&n.get(0).toString().contains("SelectionExpression(PrimaryIdentifier(\"System\"), \"out")){
			...
			if(n.get(0)==null || n.getNode(0).get(0) instanceof Node){
				variableName = "__this";
			}
			else{
			...
			if(null != n.getNode(0)){
				if(n.getNode(0).get(0) instanceof Node && n.getNode(0).getNode(0).getName().equals("CastExpression")){
					...
					if(classMatch){
						...
						variableName = n.getNode(0).getString(0);
						objectType = block.variables.get(variableName).getType();
					}
				}
			} 
				//else if(n.getNode(0).get(0) instanceof String && block.variables.containsKey(variableName)){
				//	objectType = block.variables.get(variableName).getType();
				//}
			...
			for(int i=0; i< block.classList.size(); i++){
				if(block.classList.get(i).getName().startsWith(objectType)){ 
					...
					returnType = methodInQuestion.getReturnType();
					if(methodInQuestion.isStatic){
						...
						line+= "__" + objectType + "::" + methodInQuestion.getName() + "(" + variableName;
					}
					else {
						line+= variableName + "->__vptr->" + methodInQuestion.getName() + "(" + variableName;
					}
					for(int j = 0; j < n.getNode(3).size(); j++){
						...
						else if(n.getNode(3).getNode(j).hasName("CallExpression")){
							line+="\n	({" +new ccStatement(n.getNode(3).getGeneric(j),block).publish() +";})\n";
						}
						else{
...
//		line+="}\n";
	...
	public void visitConditionalStatement(GNode n){
		line += "if("+new ccStatement((GNode)n.get(0), block, true).publish()+")";
		dispatch(n.getNode(1));
		if(n.get(2)!=null){
			line += "else ";
			dispatch(n.getNode(2));}
...
//			}
	...
	public String[] findArgumentTypes(Node n){
		...
		for(int i=0; i< n.size(); i++){
			...
			else if(n.getNode(i).getName().matches("CallExpression")){
				ccStatement state = new ccStatement(n.getGeneric(i), block);
				argTypes[i] = state.getReturnType();;
			}
		}
		return argTypes;
	}
